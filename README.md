...
lab2_measure_your_machine/ Lab 2 -- sequential/random, page cache, O_DIRECT, flush, queue depth, resize/GC, small-file tax
...
On this course's node, your shell already has `$STEAM_SERVERS` (your real
`server1`..`server8` directories), `$STEAM_METADATA` (your real
`metadata` directory), and `$STEAM_BENCH_DIR` (a scratch directory for
